...
As we can see here we are instantiating two white bishops with two Bishop instantiations, as well as providing some magic numbers to the constructor which I didn't like. The problem with the setup was that we would need an object for every piece on the board when this doesn't have to be the case.
...
This is now all the code and objects we need for the whole board. And we are successfully drawing to all the correct positions as shown below (its the same initial position image from before but it works trust me).
