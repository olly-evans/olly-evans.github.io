...
Which has been much harder than I thought it would be initially but has  also been very rewarding. Thus, this post is split into two: programming the game and complying with the rules set by the FIDE, and then eventually writing the actual engine.
...
Changing the size of the window for now works for all bar the pieces which will be smaller. As the piece images are constant. I wanted to make this dynamic but resizing images may be overkill. We'll see if it would be useful.
...
Maybe I'll change the colors later to green and red that might make more sense, could be jarring though who knows. Definitely overthinking that.
...
The shown bitboard or anything else I might want to add is appended in a sub-function of `debug()` called `debug_bitboard()`. This function is shown below.
...
It simply iterates over a seperate `debug_squares` vector held by `Board` and changes their fill color dependant on the chosen bitboard. Where `GRID_NUM_SQUARES` is a constant representing 64.
