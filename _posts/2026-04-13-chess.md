...
Looping through the pieces vector and calling the draw function we can draw the pieces appropriately. The Piece and derived-classes with a Pawn example, its like this for all other pieces bar the texture being drawn.
...
How are they positioned correctly? Good question, it worked once and now works for all of the pieces. The square index on the board is taken at instantiation, converted to 2D coordinates and multiplied by the **board_square_size** a board member.
